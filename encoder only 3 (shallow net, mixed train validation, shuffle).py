...
batch_size = 100
through = 5
file_path = saving_dir_weight+'weights.2D_classification_shallow_mixed_shuffle.best.hdf5'
checkPoint = ModelCheckpoint(file_path,monitor='val_loss',save_best_only=True,mode='min',period=1,verbose=1)
callback_list = [checkPoint]
temp_results = []
if os.path.exists(saving_dir_weight+'weights.2D_classification_shallow_mixed_shuffle.best.hdf5'):
    model_auto.load_weights(saving_dir_weight+'weights.2D_classification_shallow_mixed_shuffle.best.hdf5')
...
for ii in range(breaks):
    ...
    for jj in range(through):# going through the training data 5 times
        ...
        for kk in step_idx: # going through 10 splitted training data
            X_train_,y_train_ = pickle.load(open('D:\\NING - spindle\\Spindle_by_Graphical_Features\\data\\train\\train%d.p'%(kk),'rb'))
    ...
    model_auto.load_weights(saving_dir_weight+'weights.2D_classification_shallow_mixed_shuffle.best.hdf5')
    idx = int(X_train_.shape[0] * 0.2)
    X_ = X_train_[-idx:]
    y_ = y_train_[-idx:]
    X_predict = model_auto.predict(X_)[:,-1] > np.mean(labels[:,-1])
    X_predict_prob = model_auto.predict(X_)[:,-1]
    print(metrics.classification_report(y_[:,-1],X_predict))
    AUC = metrics.roc_auc_score(y_[:,-1], X_predict_prob)
    fpr,tpr,th = metrics.roc_curve(y_[:,-1], X_predict_prob,pos_label=1)
    sensitivity = metrics.precision_score(y_[:,-1],X_predict,average='weighted')
    selectivity = metrics.recall_score(y_[:,-1],X_predict,average='weighted')
    plt.close('all')
...
ax.set_title('Confusion matrix\nshallow net (cheat method)',fontweight='bold',fontsize=20)
